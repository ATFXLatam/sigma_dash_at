...
import { ALLOWED_DOMAINS, isAllowedDomain } from '@/config/access'
...
function RouteComponent() {
  ...
  if (!isAllowedDomain(email)) return <AccessDenied email={email} />
        ...
        <p className='max-w-md text-sm text-muted-foreground'>
          {email ? (
            <>
              ...
              outside the allowed domain ({domains}).
            </>
          ) : (
            <>Sign in with an @{domains} account to access this dashboard.</>
          )}
        </p>
      </div>
